...
def augment_cpp_type(column)
  result = case column.db_type
           when /^enum\(([^\)]+)\)/i
             ...
             "etf::utils::Fixed_size_char_array< #{size} >"                 
           when /^(?:int\((\d+)\)|int)$/i 
             column.otl_type = 'int'
             'boost::int32_t'
...
class SelectParser
  ...
  def parse_outputs(stmt)
    ...
    temp = parsed_tables.split(/\bwhere\b\s+/mi)
    if temp.length >= 2
      parsed_tables = temp[0..-2].join
    ...
    def initialize(data={ })
      ...
      tables.each_pair do | table_name, table |
        ...
        header_includes = [ 
                           ...
                           'etf/orm/otl_utils.hpp',
                           'etf/orm/orm_to_string_table.hpp',
                           'boost/thread/mutex.hpp',
                           'boost/thread/locks.hpp',
                           'pantheios/pantheios.hpp',
                           'boost/any.hpp',
                           "etf/orm/#{database_name}/connection_#{database_name}.hpp"
                          ]
        header_includes.push 'etf/utils/fixed_size_char_array.hpp' if fixed_size_char_array_required
        ...
        class_definition = {
          ...
          :template_decls => 
          [ 
           "typename PKEY_LIST_TYPE = std::list< #{table.pkey_class} >",
           "typename VALUE_LIST_TYPE = std::list< #{table.value_class} >",
...
module Codegen
  class Database
    ...
    def initialize(codegen_request={ })
      ...
      # The table classes
      ############################################################
      namespace = ['etf', 'orm', @database_name,]
      ...
      @connection_singleton_class =                      
        CppClass.new({
                       ...
                       :header_only => true,
                       :header_includes => [ 
                                            'etf/orm/connection_registry.hpp',
      ...
      if query_classes.length > 0
        Library.new(
                    { 
                      ...
                      :namespace => ['etf', 'orm', @database_name, 'query',]
                    })
      end
