...
static hid_t compound_data_type_id() { return get_instance()->compound_data_type_id_; }"
        ...
        string_group_members = group_members.select {|m| m.cpp_type =~ /Fixed_size_char_array/  }
        @string_members = []
...
^
      ...
      @log_groups.each do |group, fields|
        fields.each do |field|
          ...
          if matches.length != 1
            throw "ETF: Could not find match for field #{field} in class #{@logged_class.name}: available members\n\t" + 
